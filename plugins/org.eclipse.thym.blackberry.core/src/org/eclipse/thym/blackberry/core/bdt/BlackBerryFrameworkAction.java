...
public class BlackBerryFrameworkAction implements IPluginInstallationAction {
	...
	private final BlackBerrySDK sdk;
	
	
	public BlackBerryFrameworkAction(String src, String custom, String parent, String pluginId, File projectDir, File pluginDir, BlackBerrySDK sdk ) {
		source = new Path(src);
		this.parent = parent;
	...
	public void install() throws CoreException {
		...
		if(isCustom){
			...
			try {
				...
				BlackBerrySDKManager.getManager().updateProject(sdk, null, true, destFile, new NullProgressMonitor());
			} catch (IOException e) {
				throw new CoreException(new Status(IStatus.ERROR, BlackBerryCore.PLUGIN_ID, "Copying custom framework failed",e));
			}
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, BlackBerryCore.PLUGIN_ID, "Updating the project.properties file has failed",e));
		}		
	}
